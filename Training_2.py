...
class AppleCherryDataset(Dataset):
    """Dataset for apple and cherry instance segmentation with train/val split"""
    ...
    def __init__(self, images_dir, masks_dir, transform=None, split='train'):
        ...
        # Updated class mapping - only apple and cherry (6 classes + background = 7 total)
        self.class_map = {
    ...
    def _load_samples(self):
        """Load samples based on split - augmented for train, original for val"""
        ...
        # Walk through semantic masks for allowed fruits
        for plant_type in self.class_map.keys():
            ...
            for filename in os.listdir(plant_semantic_dir):
                ...
                if self.split == 'train':
                    # Training: use augmented images (_aug00, _aug01, etc.)
                    if not re.search(r'_aug\d+_semantic\.png$', filename):
                        continue
                    base_name = filename.replace('_semantic.png', '')
                    # Convert _aug00_semantic to _aug00 for image lookup
                    image_base = base_name
                elif self.split == 'val':
                    # Validation: use original images
                    if not filename.endswith('_original_semantic.png'):
                        continue
                    base_name = filename.replace('_semantic.png', '')
                    # Convert _original_semantic to _original for image lookup
                    image_base = base_name

                # Build paths
                semantic_path = os.path.join(plant_semantic_dir, filename)
                
                # For instance files, they follow pattern: base_name_instance.png and base_name_instances.json
        ...
        print(f"\nDataset loading complete for {self.split}:")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Parse instances - only keep apple and cherry classes
            boxes = []
            labels = []
            masks = []
            areas = []
            ...
            for inst in instance_info.get('instances', []):
                ...
                # Skip if class not in our apple/cherry mapping (1-6)
                if semantic_class_id < 1 or semantic_class_id > 6:
                    continue
    ...
    for pred in predictions:
        ...
        if keep_indices:
            ...
            processed_pred = {
                ...
                'masks': torch.empty((0, masks.shape[1], masks.shape[2]))
            }
...
def train_model(config, resume_from=None):
    ...
    print("\n" + "="*50)
    print("APPLE & CHERRY INSTANCE SEGMENTATION TRAINING")
    print("="*50)
    ...
    print(f"NMS IoU threshold: {config['nms_iou_threshold']}")
    print("="*50 + "\n")
    ...
    train_dataset = AppleCherryDataset(
        images_dir=config['images_dir'],
        masks_dir=config['masks_dir'],
        transform=transform_train,
        split='train'  # Uses augmented images
    )
    
    val_dataset = AppleCherryDataset(
        images_dir=config['images_dir'],
        masks_dir=config['masks_dir'],
        transform=transform_val,
        split='val'  # Uses original images
    )
...
def test_model_with_nms(model_path, test_image_path, device='cuda', confidence_threshold=0.5, nms_threshold=0.5):
    """Test the trained model on a single image with NMS"""
    print(f"\nTesting model on image with NMS (IoU threshold: {nms_threshold})...")
    
    # Load model
    model = get_model(num_classes=7, dropout_rate=0.0)  # 7 classes: background + 6 apple/cherry classes
    ...
    # Apply NMS
    predictions_nms = apply_nms_to_predictions(predictions, iou_threshold=nms_threshold)
    prediction = predictions_nms[0]
    ...
    print(f"Detected {len(boxes)} objects with confidence > {confidence_threshold} (after NMS)")
    ...
    # Class names mapping for apple and cherry only
    ...
    axes[1].set_title(f'Detections with NMS (threshold={confidence_threshold})')
    axes[1].axis('off')
    
    plt.tight_layout()
    
    # Save result
    result_path = test_image_path.replace('.', '_result_nms.')
...
def analyze_model_performance(model_path, val_loader, device='cuda', num_samples=50):
    """Analyze model performance on validation set with NMS"""
    print("\nAnalyzing model performance with NMS...")
    ...
    with torch.no_grad():
        for idx, (images, targets) in enumerate(val_loader):
            ...
            try:
                ...
                predictions_nms = apply_nms_to_predictions(predictions, iou_threshold=0.5)
                
                all_predictions.extend(predictions_nms)
...
def main():
    ...
    config = {
        ...
        'save_dir': 'apple_cherry_model_enhanced',
        
        # Model parameters - only apple and cherry (6 classes + background = 7 total)
        'num_classes': 7,
        'dropout_rate': 0.3,
    ...
    # List plant types found (should only show apple and cherry)
    if os.path.exists(semantic_dir):
        ...
        # Show sample files for apple and cherry
        for plant in ['apple', 'cherry']:
            plant_dir = os.path.join(semantic_dir, plant)
            if os.path.exists(plant_dir):
                files = os.listdir(plant_dir)
                aug_files = [f for f in files if '_aug' in f and '_semantic.png' in f]
                orig_files = [f for f in files if '_original_semantic.png' in f]
                print(f"  {plant}: {len(aug_files)} augmented, {len(orig_files)} original")
    ...
    try:
        ...
        # Look for original images for testing (validation set)
        ...
        if test_images:
            print(f"\nTesting on {len(test_images)} sample images with NMS...")
