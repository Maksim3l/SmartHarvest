...
def create_augmented_dataset(original_images_dir, masks_dir, augmented_output_dir, 
                           augmentation_params=None, augmentations_per_image=18):
    """
        ...
        for filename in files:
            if filename.endswith('_semantic.png'):
                ...
                try:
                    ...
                    # Load the original instance info JSON once
                    info_path = os.path.join(instance_masks_dir, plant_type, f"{base_name}_instances.json")
                    original_info_data = None
                    
                    if os.path.exists(info_path):
                        with open(info_path, 'r') as f:
                            original_info_data = json.load(f)
                        print(f"Loaded JSON info for {base_name}")
                    else:
                        print(f"WARNING: No JSON info found at {info_path}")
                    
                    # FIX: Copy instance info JSON for the original version too!
                    if original_info_data:
                        # Create copy for original
                        info_data = original_info_data.copy()
                        info_data['original_filename'] = info_data['filename']
                        info_data['filename'] = f"{base_name}_original{orig_ext}"
                        info_data['augmentations'] = []  # No augmentations for original
                        
                        # Save original JSON
                        original_json_path = os.path.join(aug_instance_dir, plant_type, f"{base_name}_original_instances.json")
                        with open(original_json_path, 'w') as f:
                            json.dump(info_data, f, indent=2)
                        print(f"Saved original JSON: {original_json_path}")
                    ...
                    for aug_idx in range(augmentations_per_image):
                        ...
                        # Copy instance info JSON with updated filename for augmented versions
                        if original_info_data:
                            info_data = original_info_data.copy()
                            ...
                            aug_json_path = os.path.join(aug_instance_dir, plant_type, f"{base_name}{aug_suffix}_instances.json")
                            with open(aug_json_path, 'w') as f:
                                json.dump(info_data, f, indent=2)
                            print(f"Saved augmented JSON: {aug_json_path}")
                        else:
                            print(f"WARNING: Skipping JSON for {base_name}{aug_suffix} - no original JSON data")
